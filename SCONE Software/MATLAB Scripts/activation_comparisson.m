...
for i = (fix(l/10)):1:l
   ...
   ground_force_l(i-(fix(l/10))+1)= array(i,8);
   ground_force_r(i-(fix(l/10))+1)= array(i,9);
...
%RIGHT
for p = 1 : 1 : length(heel_strike_r) % p = amount of heelstrikes
    %resize the shape of the curves
    if (p == length(heel_strike_r))
        
        temp_hamstring_r = hamstring_r(pre_last_r : last_r);
        new_hamstring_r(p,:) = imresize(temp_hamstring_r, [1,length(last_cycle_time_r)]);

        temp_glutmax_r = glutmax_r(pre_last_r : last_r);
        new_glutmax_r(p,:) = imresize(temp_glutmax_r, [1,length(last_cycle_time_r)]);

        temp_iliopsoas_r = iliopsoas_r(pre_last_r : last_r);
...
%LEFT
for p = 1 : 1 : length(heel_strike_l) % p = amount of heelstrikes
    
    if (p == length(heel_strike_l))
        
        %resize the shape of the curves
        temp_hamstring_l = hamstring_l(pre_last_l : last_l);
        new_hamstring_l(p,:) = imresize(temp_hamstring_l, [1,length(last_cycle_time_l)]);

        temp_glutmax_l = glutmax_l(pre_last_l : last_l);
        new_glutmax_l(p,:) = imresize(temp_glutmax_l, [1,length(last_cycle_time_l)]);

        temp_iliopsoas_l = iliopsoas_l(pre_last_l : last_l);
...
legend('Hamstrings activation of the right leg','Hamstrings activation of the left leg')
...
legend('Gluteus Maximus activation of the right leg','Gluteus Maximus activation of the left leg')
...
legend('Iliopsoas activation of the right leg','Iliopsoas activation of the left leg')
