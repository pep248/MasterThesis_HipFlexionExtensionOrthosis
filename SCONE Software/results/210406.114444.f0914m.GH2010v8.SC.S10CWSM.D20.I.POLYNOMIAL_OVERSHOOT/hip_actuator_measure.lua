...
function result( model )
	effort = (sum_r*sum_r + sum_l*sum_l) / (2 * steps) -- maybe we should try to maximise this?
	return effort
end

function store_data( frame )
	frame:set_value("hip_flexion_actuator_r", actuator_r:input()/100);
	frame:set_value("hip_flexion_actuator_l", actuator_l:input());
	-- frame:set_value("state", statestate);
