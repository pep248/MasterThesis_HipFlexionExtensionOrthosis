...
function init( model, par )
	...
	current_period = period --period after the control scheme
	real_current_period = period -- duration of the last step
...
function update( model )
	...
	if (state_force == 0) then -- we use this state machine to aboid some unpredictable bouncing of the feet counting as 2 steps
		if ( (previous_ground_reaction_force_right == 0) and (current_ground_reaction_force_right ~= 0) ) then
			...
			real_current_period = current_heel_strike_force - last_heel_strike_force
			--control scheme
	...
	-- get rid of time_offset (try to run everything on gait %)
		...
		time = (model:time()-time_offset)*(period/current_period) -- we compute the escalated time
		...
		if ( (model:time()-time_offset) < current_period ) then -- we apply the torque only for a short time, to aboid the SUPER torques of a polynomial
			torque = ( torque_multiplier *( p4*(time)^4 + p3*(time)^3 + p2*(time)^2 + p1*(time) + p0 ) )
...
function store_data( frame )
	...
	frame:set_value("real_current_period", real_current_period);
	frame:set_value("period_ratio", period/current_period);
