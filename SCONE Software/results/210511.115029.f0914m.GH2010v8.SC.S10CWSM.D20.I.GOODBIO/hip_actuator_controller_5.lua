...
function init(model, par)
    ...
    performed_gait = 1 -- tanperone of gait 
    temp_performed_gait = 1
    real_performed_gait = 1
...
function update(model)
    ...
    if (state_force == 0) then -- we use this state machine to aboid some unpredictable bouncing of the feet counting as 2 steps
        if ((previous_ground_reaction_force_right == 0) and (current_ground_reaction_force_right ~= 0)) then
            ...
            if (first_step == 1) then -- we execute this just once we wave the first heel strike
                first_step = 2
            elseif (first_step == 0) then -- we execute this just once we wave the first heel strike
                first_step = 1
            end
        end
    ...
    if (first_step ~= 0) then -- We start when we detect the first heel-strike
		time = (model:time() - time_offset + performed_gait*new_period) * ( original_period / new_period );
		sine_torque = torque_multiplier*(a1 * math.sin(b1 * time + c1) + a2 * math.sin(b2 * time + c2) + a3 * math.sin(b3 * time + c3) + a4 * math.sin(b4 * time + c4) + a5 * math.sin(b5 * time + c5) + a6 * math.sin(b6 * time + c6) + a7 * math.sin(b7 * time + c7) + a8 * math.sin(b8 * time + c8) )
        actuator_r:add_input(sine_torque)

    elseif (first_step == 1) then
        time = (model:time() - time_offset) * ( original_period / new_period );
        sine_torque = torque_multiplier*(a1 * math.sin(b1 * time + c1) + a2 * math.sin(b2 * time + c2) + a3 * math.sin(b3 * time + c3) + a4 * math.sin(b4 * time + c4) + a5 * math.sin(b5 * time + c5) + a6 * math.sin(b6 * time + c6) + a7 * math.sin(b7 * time + c7) + a8 * math.sin(b8 * time + c8) )
        actuator_r:add_input(sine_torque)
    end
